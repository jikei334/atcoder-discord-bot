...
- AtCoder ID を Discord ユーザーに紐づける
...
- フロー：
  ...
  2. 解いた問題選択（コンテストの問題数に合わせた動的なトグルボタン）
  3. 一言感想入力（任意、テキスト入力）
  4. 送信
...
AtCoder には公式 API がないため、以下を利用する：
- コンテスト一覧: `https://kenkoooo.com/atcoder/resources/contests.json`（AtCoder Problems）
